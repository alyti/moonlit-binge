...
async fn splice(
    ...
    for content in &params.contents {
        let dir = tokio::fs::try_exists(single_base_path.join(format!("{}", content.connection)).join(&content.content_id).join("main.m3u8")).await?;
        if !dir {
    ...
    for content in params.contents.into_iter() {
        let single_content_dir = single_base_path.join(format!("{}", content.connection)).join(&content.content_id);
        ...
        for variant in manifest.variants.into_iter() {
            ...
            for segment in media.segments.iter_mut() {
                segment.uri = format!("../../single/{}/{}/{}", content.connection, content.content_id, segment.uri);
    ...
    spliced_master.expect("Missing spliced main file").write_to(&mut v).unwrap();
    tokio::fs::write(playlist_base_path.join("main.m3u8"), v).await?;

    for (name, media) in spliced_media.expect("Missing spliced media files").iter_mut() {
        let mut v: Vec<u8> = Vec::new();
